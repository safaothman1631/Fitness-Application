...
if (Test-Path $androidSdkPath) {
    ...
    try {
        ...
        Write-Host ""
        Write-Host "⚠️  IMPORTANT: You need to restart VS Code for changes to take effect!" -ForegroundColor Yellow
        Write-Host ""
    }
    catch {
        Write-Host "❌ Error setting permanent environment variables: $_" -ForegroundColor Red
    ...
    if (Test-Path "$androidSdkPath\platform-tools\adb.exe") {
        ...
        Write-Host "⚠️  ADB not found - you may need to install it via Android Studio SDK Manager" -ForegroundColor Yellow
    }
