...
export async function GET(request: NextRequest) {
  try {
    console.log('🎬 Fetching workout videos from Firebase Storage...')
    
    const bucket = adminStorage.bucket()
    const folderPath = 'workouts/videos/'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const bucket = adminStorage.bucket()
    const filePath = `workouts/videos/${fileName}`
    ...
    console.log(`✅ Uploaded workout video: ${fileName}`)
    ...
    return NextResponse.json(
      { error: 'Failed to upload workout video', details: error.message },
      { status: 500 }
    )
  }
}
