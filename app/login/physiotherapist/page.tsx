...
export default function PhysiotherapistLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (formData.email === "physio@darinfitness.com" && formData.password === "physio123") {
        localStorage.setItem("isAuthenticated", "true")
        localStorage.setItem("userRole", "physiotherapist")
        toast.success("Login successful!")
        router.replace("/physiotherapist")      } else {
  ...
  <div className="min-h-screen flex flex-col bg-slate-950 text-white relative overflow-hidden">
          ...
          <div 
              ...
              style={{
                ...
                backgroundSize: "140% auto",
                  backgroundImage: "url('/images/bg-physiotherapy-mobile.svg')",
                  animation: "subtleFloat 20s ease-in-out infinite"
              }}
          />
          {/* Gradient Overlay for depth */}
          <div className="absolute inset-0 bg-gradient-to-b from-green-950/55 via-slate-950/45 to-slate-950/70" />
      </div>
      
      {/* Header block (now wraps the card background area too) */}
      <div className="relative z-10 bg-gradient-to-b from-green-600/20 via-transparent to-transparent pb-24">
        <AuthTopbar />
        <div className="max-w-md mx-auto px-4 pb-10 pt-3 text-center">
          ...
          <h1 className="text-3xl font-bold tracking-tight">Physiotherapy Login</h1>
          <p className="text-slate-100/80 mt-2 text-sm">Access your dashboard with your credentials</p>
        </div>
        {/* Card sits inside green block so green is truly behind it */}
          <div className="max-w-md mx-auto px-6">
              <Card className="bg-slate-900/80 backdrop-blur-xl border-green-500/30 py-8 rounded-3xl shadow-2xl ring-1 ring-green-400/20">
            <CardContent className="px-6">
              <form onSubmit={handleLogin} className="space-y-6">
                ...
                <Button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-green-600 hover:bg-green-600/90 h-12 rounded-[14px] font-semibold tracking-wide inline-flex items-center justify-center gap-2"
                >
      ...
      {/* Body area after the green header block */}
